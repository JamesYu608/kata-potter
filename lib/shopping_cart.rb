...
class ShoppingCart
  ...
  def is_edge_case(book_to_count)
    return false if book_to_count.keys.size != 5

    book_to_count = book_to_count.dup
    remove_one_book_from_each_group!(book_to_count)
    return false if book_to_count.keys.size != 3
